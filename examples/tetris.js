function setup() {
  ...
  for(var i = 0; i < 10; i++){
    var o0 = Math.floor(random()*possible.length);
    var o1 = Math.floor(random()*possible.length);
    if(o0 === o1){
      o1+=1;
      o1%=7;
    }
...
var size;
var speeds = [0.01667,0.021017,0.026977,0.035256,0.04693,0.06361,0.0879,0.1236,0.1775,0.2598,0.388,0.59,0.92,1.46,2.36];
var speed = speeds[0];
...
var tetronimos = [];
var possible = [[{x:-2,y:0},{x:-1,y:0},{x:0,y:0},{x:1,y:0}],[{x:-1,y:0},{x:0,y:0},{x:1,y:0},{x:1,y:1}],[{x:-1,y:1},{x:0,y:0},{x:1,y:0},{x:-1,y:0}],[{x:-1,y:0},{x:0,y:0},{x:0,y:1},{x:1,y:1}],[{x:-1,y:1},{x:0,y:0},{x:0,y:1},{x:1,y:0}],[{x:-1,y:1},{x:-1,y:0},{x:0,y:1},{x:0,y:0}],[{x:-1,y:0},{x:0,y:0},{x:0,y:1},{x:1,y:0}]];
var stored = [];
var vars = ['speed','l','score','tetronimos','stored','totalLines'];
...
function draw() {
  ...
  for(var i = 0; i < stored[0].length; i++){
    fill(255);
...
class tetronimo{
      ...
      for(var i = 0; i < rows.length; i++){
        if(rows[i].length === 10){
          for(var r = 0; r < rows[i].length; r++){
            this.devare.push(this.rects.indexOf(rows[i][r]));
          }
          devaredRows++;
        }else{
          for(var r = 0; r < rows[i].length; r++){
            this.rects[this.rects.indexOf(rows[i][r])].y-=devaredRows;
          }
        }
      }
      if(devaredRows!==0){
        totalLines+=devaredRows
        score+=scores[devaredRows-1]*(l+1);
  ...
  update(){
    ...
    if(this.devare.length!==0){
      sort(this.devare);
      for(var i = this.devare.length-1; i >= 0; i--){
        this.rects.splice(this.devare[i],1);
      }
    }
  ...
  combine(other){
    for(var i = 0; i < other.rects.length; i++){
  ...
  checkCollision(){
    ...
    for(var i = 0; i < 1 && !this.down; i++){
      if(tetronimos[i]!==this){
        for(var r = 0; r < this.rects.length && !this.down; r++){
          for(var o = 0; o < tetronimos[i].rects.length && !this.down; o++){
  ...
  checkRotate(){
    ...
    for(var i = 0; i < 1 && !this.down; i++){
      if(tetronimos[i]!==this){
        for(var r = 0; r < newRects.length && !this.down; r++){
          for(var o = 0; o < tetronimos[i].rects.length && !this.down; o++){
  ...
  rotate(){
    for(var i = 0; i < this.rects.length; i++){
      var x = this.rects[i].x;
      var y = this.rects[i].y;
